...
class DiscoveryListResource(Resource):
    ...
    def get(self):
        ...
        print("User PREFERENCE : ", user.preferences)
        print(type(user.preferences) is not list)
        user_is_bisexual = len(user.preferences) > 1 or "other" in user.preferences if type(user.preferences) is list else True
        ...
        if user_is_bisexual:
            ...
            gender_to_find = "male" if user.preferences[0] == "male" else "female"
            query = """
                SELECT
                users.id as id,
                fname,
            ...
            with connection.cursor() as c:
                c.execute(query, (user.latitude, user.longitude, user.latitude, user.id, user.id, user.id, gender_to_find, distance, skip, take))
                pool.release(connection)
                return matches.filter(c.fetchall()), 200
